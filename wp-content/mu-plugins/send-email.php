...
add_action('init', function () {
    if (isset($_GET['send-email'])) {
                ...
                if ($uid || !get_user_meta($id, $meta)) {
                    ...
                    $email = [
                        ...
                        'content' => "<font face=sans-serif>Salut à tous,
                            <b>La prochaine Vélorution Metz aura lieu le " . $date . " !</b>
                    ...
                    if (!$dr) {
                        wp_mail($email['to'], $email['sujet'], $email['content']);
        ...
        echo $cpt . ' emails envoyés pour ' . count($users) . ' users';
        exit;
        print_r($_GET);
        exit;
    }
